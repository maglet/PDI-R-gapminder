...
```{r}
phrase<-"This is not in a comment" #This text will be assigned to the variable y
...
```{r}
barplot(height=gapminder[1,37:48],                     #height of the bars 
...
barx <- barplot(height=gapminder[1,37:48],,     #height of the bars 
...
error.bar(barx, gapminder[1,37:48], colsds)
```
...
Wow, asia has a lot of countries with outliers. We can zoom on on the box plot ysing the y lim parameter.
...
plot(gapminder$lifeExp, gapminder$year,
     xlab="Index",        #X axis label
     ylab= "Life expectancy",    #Y axis label (somehow get )
     las= 1,                  #Make numbers horizontal on the x axis, 
     cex.lab=1.2,             #Increase axis labels font size
     cex.axis=1.2,            #Increase axis label number size
     lwd=2,                   #Increase line/dot thickness
